(function () {
  ...
  function getCompletions(token, context, keywords, options) {
    ...
    function gatherCompletions(obj) {
		...
		var xx = Object.getOwnPropertyNames(obj);
	...
	function checkImportDefs(ctx){
		if(options.scopes){
			for(var i=0; i<options.scopes.length; i++){
				var base = options.scopes[i];
				for(var j=context.length - 1; (j >= 0 && base != null); j--){
					base = base[context[j].string];
				}
				if(base){
    ...
    if (context) {
      ...
      if (obj.type.indexOf("variable") === 0) {
		  gatherCompletions(Import[obj.string]);
      ...
      if (base != null) gatherCompletions(base);
		...
		if(options.scopes){
			for(var i=0; i<options.scopes.length; i++){
				gatherCompletions(options.scopes[i]);
			}
		}
		/*if(found.length > 0){
			return found;
